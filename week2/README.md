...
The `u.item` parser treated the `unknown` flag as the first real genre. That shifted every genre label by one and dropped Western. The original recommender also used Jaccard similarity for just one selected movie.
...
The first checks the genre parser and compares the old Jaccard results with `fixed/`. The second verifies a cosine score by hand and checks duplicate handling. The analysis script compares item-to-item and profile recommendations, then writes `results/recommendation-analysis.md`.
